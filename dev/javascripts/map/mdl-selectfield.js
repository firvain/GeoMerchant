function handleSelect()  {
  ...
  window.MaterialSelectfield = e, e.prototype.CssClasses_ = {
    ...
    this.options_ = [], this.optionsMap_ = {}, this.optionsArr_ = [], this.closing_ = !0, this.keyDownTimerId_ = null
  ...
  }, e.prototype.enable = e.prototype.enable, e.prototype.toggle = function(e) {
    this.element_.classList.contains(this.CssClasses_.IS_FOCUSED) ? this.hide_() : this.show_(e)
    ...
    if (this.checkDisabled(), !this.element_.classList.contains(this.CssClasses_.IS_DISABLED)) {
      ...
      var t = this.listOptionBox_.querySelector('.' + this.CssClasses_.IS_SELECTED);
    ...
    if (t && t.length > 0 && !this.closing_) {
      var s,
          i = Array.prototype.slice.call(t).indexOf(this.listOptionBox_.querySelectorAll('.' + this.CssClasses_.IS_SELECTED)[0]);
    ...
    if (this.element_) {
      this.label_ = this.element_.querySelector('.' + this.CssClasses_.LABEL), this.select_ = this.element_.querySelector('.' + this.CssClasses_.SELECT);
      var e = document.createElement('div');
      e.classList.add(this.CssClasses_.SELECTED_BOX), e.tabIndex = 1, this.selectedOption_ = e;
      var t = document.createElement('i');
      t.classList.add('material-icons'), t.tabIndex = -1, t.textContent = 'keyboard_arrow_down', e.appendChild(t);
      ...
      if (this.select_ && (this.options_ = this.select_.querySelectorAll('option'), this.select_.style.visibility = 'hidden', this.options_.length)) {
        this.boundSelectedHandler = this.onSelected_.bind(this);
        var o = document.createElement('div');
        o.classList.add(this.CssClasses_.LIST_OPTION_BOX), o.tabIndex = '-1';
        var n = document.createElement('ul');
        n.tabIndex = '-1';
        for (var l = 0; l < this.options_.length; l++) {
          var a = this.options_[l],
              d = (a.textContent || '').toUpperCase().replace(/( )|(\n)/g, '');this.optionsMap_[d] = l, this.optionsArr_.push(d);var c = document.createElement('li');c.textContent = a.textContent, c.setAttribute('data-value', l), c.tabIndex = '-1', c.addEventListener('click', this.boundSelectedHandler), n.appendChild(c), a.selected && '' !== a.textContent && (this.element_.classList.add(this.CssClasses_.IS_DIRTY), this.selectedOptionValue_.textContent = a.textContent, c.classList.add(this.CssClasses_.IS_SELECTED))
        }
        o.appendChild(n), this.element_.appendChild(o), this.listOptionBox_ = o
      }
      this.boundClickHandler = this.onclick_.bind(this), this.element_.addEventListener('click', this.boundClickHandler), i && this.element_.classList.add(this.CssClasses_.IS_INVALID), this.checkDisabled()
    }
  }, e.prototype.mdlDowngrade_ = function() {
    if (this.element_.removeEventListener('click', this.boundClickHandler), this.boundSelectedHandler && this.options_.length)
      for (var e = this.listOptionBox_.querySelectorAll('li'), t = 0; t < e.length; t++) e[t].removeEventListener('click', this.boundSelectedHandler)
